...
class DataAdapter: RecyclerView.Adapter<BaseViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        return if(viewType == TYPE_ITEM) ItemViewHolder.from(parent) else SectionViewHolder.from(parent)
    }
...
class ItemViewHolder(private val itemRowBinding: ItemRowBinding): BaseViewHolder(itemRowBinding.root) {
    companion object {
...
class SectionViewHolder(private val sectionBinding: SectionBinding): BaseViewHolder(sectionBinding.root) {
    companion object {
